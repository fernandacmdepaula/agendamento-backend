...
APP_VERSION = "2026-04-29-improved-event-title-v1"
...
def build_event_title(client_name: str, service_name: str) -> str:
    name = client_name.strip() if client_name else "Cliente"
    service = service_name.strip().title() if service_name else ""
    if service:
        return f"✂️ {name} - {service}"
    return f"✂️ {name}"
...
def find_conflicts(service, start_time: str, end_time: str, exclude_google_event_id: Optional[str] = None) -> List[Dict[str, Any]]:
    try:
...
def format_db_datetime(start_time: str) -> Tuple[str, str]:
    ...
    return dt_for_db.strftime("%Y-%m-%d"), dt_for_db.strftime("%Y-%m-%d %H:%M:%S")


@app.middleware("http")
async def log_requests(request: Request, call_next):
    body = await request.body()
    logger.info("request path=%s method=%s content-type=%s body_len=%s",
        request.url.path, request.method, request.headers.get("content-type"), len(body))
...
async def debug_latest():
    ...
    resp = service.events().list(calendarId=CALENDAR_ID, timeMin=now, maxResults=10, singleEvents=True, orderBy="startTime").execute()
    return {"calendar_id": CALENDAR_ID, "count": len(resp.get("items", [])), "events": [
        {"id": ev.get("id"), "summary": ev.get("summary"), "start": ev.get("start"), "end": ev.get("end"), "status": ev.get("status"), "htmlLink": ev.get("htmlLink")}
        for ev in resp.get("items", [])
    ]}
...
async def debug_zaia(days_back: int = 60, days_forward: int = 180, max_results: int = 50):
    ...
    resp = service.events().list(
        ...
        q="[ZAIA]", maxResults=max_results, singleEvents=True, orderBy="startTime"
    ).execute()
    return {"calendar_id": CALENDAR_ID, "count": len(resp.get("items", [])), "events": [
        {"id": ev.get("id"), "summary": ev.get("summary"), "start": ev.get("start"), "end": ev.get("end"), "status": ev.get("status"), "htmlLink": ev.get("htmlLink")}
        for ev in resp.get("items", [])
...
async def booking_created(request: Request):
    ...
    booking_id = (data.get("booking_id") or data.get("id") or "").strip() or str(uuid.uuid4())
    ...
    event = {
        "summary": build_event_title(client_name, service_name),
        "start": {"dateTime": start_time},
        "end": {"dateTime": end_time},
        "description": f"📱 Telefone: {client_phone}\n🆔 Booking ID: {booking_id}\n{notes}".strip(),
    }
    ...
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO appointments (booking_id, google_event_id, status, client_phone, start_date, start_at, reminder_sent, reminder_sent_at)
